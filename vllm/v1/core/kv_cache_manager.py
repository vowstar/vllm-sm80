    ...
    def truncate_computed_blocks(
        ...
        for group_blocks, manager, group in zip(
            ...
            if not group.kv_cache_spec.participates_in_prefix_caching:
                # Scratch groups (e.g. the CSA compressor ring, the GLM kpool
                # tail) hold a fixed block covering no token range, so a lookup
                # result never carries computed blocks for them and their block
                # size does not divide the endpoint.
