...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def _remove_blocks_in_range(
        ...
        if first_block >= last_block:
            return
        freed: list[KVCacheBlock] = []
        # Mamba prefill leaves null gaps between states awaiting retirement, so
        # the base implementation's `break` on the first null strands every
        # older state behind it.
        for i in range(last_block - 1, first_block - 1, -1):
            if blocks[i].is_null:
                continue
            freed.append(blocks[i])
