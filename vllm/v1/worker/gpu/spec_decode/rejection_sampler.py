...
class RejectionSampler:
    ...
    def _verify(
        ...
        # vllm#54442: verification may run against a copy with some drafts
        # invalidated; penalties and bad-words keep the real ids.
    ...
    def __call__(
        ...
        # vllm#54442: these positions belong to a structured-output request
        # whose bitmask rows past the first carry the all-permissive
        # `_full_mask`. Marking their drafts invalid (`is_valid_draft =
        # draft_sampled >= 0`) pins the accepted length so sampling never
        # reaches an unconstrained row. Only verification sees the
        # invalidated copy; `apply_sampling_params` keeps the real draft ids
        # so penalties and bad-words are unaffected.
        verify_draft_sampled = draft_sampled
