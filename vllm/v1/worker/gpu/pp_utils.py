...
def compute_need_sampled_mask(input_batch: InputBatch) -> np.ndarray | None:
    ...
    The only sound exclusion is a non-final prefill chunk, which provably
    produces no sample. Whether a request stops is only known after sampling,
    and under speculative decoding num_computed_tokens advances several tokens
    per step, so it can overrun prompt_len + max_tokens while the scheduler is
    still running the request; predicting "finished" here freezes the earlier
    stages' last_sampled_tokens and draft_tokens while the last rank keeps
    advancing its own, and the stages diverge permanently (vllm#54436)."""
