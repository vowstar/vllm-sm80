...
def convert_to_nvfp4_moe_kernel_format(
    ...
    if nvfp4_backend == NvFp4MoeBackend.B12X:
        ...
        if use_a16:
            ...
            if (
                torch.unique(a13_scale).numel() != 1
                or torch.unique(a2_scale).numel() != 1
            ):
                logger.warning_once(
                    "In NVFP4 linear, the activation global scale for inputs are "
                    "different for MOE w13 (gate_up_proj) layer or MOE w2 "
                    "(down_proj). Using a13_scale = a13_scale.max() and "
                    "a2_scale = a2_scale.max()."
                )

            # moe_kernel_quantize_input -> ref_nvfp4_quant_dequant uses the
            # inverse ModelOpt input scale directly (large global scale).
